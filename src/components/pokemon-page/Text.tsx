...
function Text({ pokemon, species }: Props): ReactElement {
    ...
    return (
        <>
            <p className="text-black text-4xl text-center font-bold">{pokemon.name.slice(0, 1).toUpperCase() + pokemon.name.slice(1) + " (" + species.genera.find((entry: any) => entry.language.name === `en`).genus + ")"}</p>
